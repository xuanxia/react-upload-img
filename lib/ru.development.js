...
var _message2 = __webpack_require__("antd/lib/message");
...
__webpack_require__("antd/lib/modal/style");

__webpack_require__("antd/lib/upload/style");

__webpack_require__("antd/lib/icon/style");

__webpack_require__("antd/lib/message/style");
...
var PictureUploader = function (_Component) {
        ...
        _this.beforeUpload = function (file, fileList) {
            ...
            return new Promise(function (resolve, reject) {
                if (state.fileList.length + fileList.length > totalNum) {
                    _message3.default.warn('\u6700\u591A\u4E0A\u4F20' + totalNum + '\u5F20\u56FE\u7247 \u8FD8\u80FD\u4E0A\u4F20' + (totalNum - state.fileList.length) + '\u5F20');
                    reject();
                    return false;
                }

                if (!suffixMap[file.type]) {
                    _message3.default.error('只能上传图片文件，可以是jpg/png/gif/bmp/webp格式!');
                    reject();
                    return false;
                }

                if (file.size / 1024 > (maxSize || defaultMaxSize)) {
                    _message3.default.error('\u56FE\u7247\u5927\u5C0F\u4E0D\u80FD\u8D85\u8FC7' + (maxSize || defaultMaxSize) + 'kb!');
                    reject();
                    return false;
                }
    ...
    _createClass(PictureUploader, [{
        ...
        key: 'getImages',
        value: function getImages(deleteFileName) {
        ...
        key: 'getPreNextInfo',
        value: function getPreNextInfo(fileName) {
            var fileList = this.state.fileList;
        ...
        value: function showPreImage(index) {
            var fileList = this.state.fileList;

            var nextIndex = index - 1;
            var file = fileList[nextIndex];

            var _getPreNextInfo = this.getPreNextInfo(file.name),
                currentIndex = _getPreNextInfo.currentIndex,
                needPre = _getPreNextInfo.needPre,
                needNext = _getPreNextInfo.needNext;
